...
class ReopenWorkJobController extends Controller
{
    ...
    {
        $this->abortIfWorker($request, 'Workers cannot reopen work jobs.');
        ...
        try {
            ...
            $workJob = $this->workJobService->reopen($workJob, $request->user(), $validated['remarks'] ?? null);

            return response()->json([
                'message' => 'Work job reopened.',
                'data' => new WorkJobResource($workJob),
            ]);
        } catch (InvalidStatusTransitionException $e) {
            return response()->json(['message' => $e->getMessage()], 422);
        } catch (Throwable $e) {
            Log::error('Failed to reopen work job', ['work_job_id' => $workJob->id, 'error' => $e->getMessage()]);
