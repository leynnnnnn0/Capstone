...
class ConfirmWorkJobController extends Controller
{
    ...
    public function __invoke(Request $request, WorkJob $workJob): JsonResponse
    {
        ...
        try {
            ...
            $workJob = $this->workJobService->confirm($workJob, $request->user(), $validated['remarks'] ?? null);

            return response()->json([
                'message' => 'Work job confirmed.',
                'data' => new WorkJobResource($workJob),
            ]);
        } catch (InvalidStatusTransitionException $e) {
            return response()->json(['message' => $e->getMessage()], 422);
        } catch (Throwable $e) {
            Log::error('Failed to confirm work job', ['work_job_id' => $workJob->id, 'error' => $e->getMessage()]);
