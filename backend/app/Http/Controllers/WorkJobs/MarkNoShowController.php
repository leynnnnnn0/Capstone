...
class MarkNoShowController extends Controller
{
    ...
    public function __invoke(Request $request, WorkJob $workJob): JsonResponse
    {
        ...
        try {
            ...
            $workJob = $this->workJobService->markNoShow($workJob, $request->user(), $validated['remarks'] ?? null);

            return response()->json([
                'message' => 'Work job marked as no show.',
                'data' => new WorkJobResource($workJob),
            ]);
        } catch (InvalidStatusTransitionException $e) {
            return response()->json(['message' => $e->getMessage()], 422);
        } catch (Throwable $e) {
            Log::error('Failed to mark work job no show', ['work_job_id' => $workJob->id, 'error' => $e->getMessage()]);
