...
def build(i):
    if i==1:
        ...
        run(2.0,.58,bays=1)
        with m.place('Perpendicular stair return',x=1.0,y=1.16,z=-.75,angle=90):
            run(1.5,.58,bays=1,skip_first=True)
    elif i==3:run(3.2,bays=4)
    elif i==4:
        run(4.0,mat=m.BLACK,bottom=True,bays=4)
        with m.place('Balcony corner return',x=2.0,z=-1.0,angle=90):
            run(2.0,mat=m.BLACK,skip_first=True,bottom=True,bays=2)
    elif i==5:run(2.4,.62)
